...
**2026-05-11:** A raised stop, placed at a prior swing level, will fire on normal retracement — that is intentional and correct. GOOGL's stop at $391 (set below $400 high after 52-week high breakout) triggered when GOOGL pulled back to $389.23 on the return from the weekend. The stop did its job: it protected entry-level capital on a position that failed to build on its breakout. Do not second-guess a triggered stop after the fact. Also: EOD stop scans can catch triggers after market close when execution is impossible — ensure the pre-market session the following morning opens with a queued limit sell before doing any other analysis. Stop-triggered sells have priority over new research.
